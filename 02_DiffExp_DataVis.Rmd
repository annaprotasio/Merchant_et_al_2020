...
# rename levels of life cycle stages
...
# rename levels of Atlas
levels(Ac_RPKM.m2$Var1)[levels(Ac_RPKM.m2$Var1)=="s0020.g108"] <- "Atlas"

t.goi.m2 = Ac_RPKM.m2[which(Ac_RPKM.m2$Var1 %in% goi.n),]

ggplot() +
  geom_boxplot(data = Ac_RPKM.m2, aes(x = factor(Var2), y= value), fill = "lightgrey") + # removes outliers, use with scale_y_continuous(limits = c(lower, upper))
  # points of data.frame literal
  geom_point(data = t.goi.m2, size = 4, aes(x=factor(Var2), y=value, color = Var1, shape = Var1)) +
...
levels(goi.1)[levels(goi.1)=="s0020.g108"] <- "Atlas"
t.goi.1 = Ac_RPKM.m2[which(Ac_RPKM.m2$Var1 %in% goi.1),]
t.goi.1 = droplevels(t.goi.1)
t.goi.1$Var1 = factor(t.goi.1$Var1, levels = sort(levels(t.goi.1$Var1)))

line.atlas = t.goi.1[which(t.goi.1$Var1=="Atlas"),]

  ggplot() +
  geom_boxplot(data = Ac_RPKM.m2, aes(x = factor(Var2), y= value), fill = "lightgrey", 
               outlier.shape = NA) + # removes outliers, use with scale_y_continuous(limits = c(lower, upper))
  geom_point(data = t.goi.1, size = 4, aes(x=factor(Var2), y=value, color=Var1, shape = Var1), position = position_jitterdodge(jitter.width = 0)) + 
